...
const CartTable = ({ setAmount }: CartTableProps) => {
  ...
  const { data: cartData, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      setAmount(
        response.data.data.carts.reduce(
        ...
        <TableBody>
          {isLoading ? (
            ...
            cartData.data.carts.map((item: any, index: number) => (
              <TableRow
                key={index}
                className={item.productItemID.stock === 0 ? 'opacity-50' : ''}
              >
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex flex-col gap-y-2 justify-center">
                      ...
                      <button
                        ...
                        onClick={() => mutate({ productId: item.productID._id, productItemId: item.productItemID._id })}
